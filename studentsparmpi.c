...
int main(int argc, char **argv)
{
	...
	if(myrank == 0) {
		...
		matrizNotas = (int*) malloc(R * C * A * sizeof(int));
	...
	*Brasil = calloc(1, sizeof(estogram));
	...
	double *dados_brasil = (double *) malloc(3 * sizeof(double));
	...
	if(myrank == 0) {
		...
		calcula_metricas_pais(*Brasil, dados_brasil, fullR*C*A);
